...
Przez wzgląd na wcześniej nabyte umiejętności tworzenia aplikacji za pomocą języka Dart przez jednego z członków sekcji, prace nad projektem rozpoczęto od implementacji podstawowej wersji aplikacji realizującej proste przesyłanie informacji w postaci całkowitoliczbowej w celu późniejszej interpretacji otrzymanych danych w środowisku Arduino. 

Przez wzgląd na tematykę projektu implementacja aplikacji nie zostanie w poniższym raporcie szczegółowo omówiona. Zostanie przedstawiony podstawowy interfejs graficzny oraz zasada działania kluczowych funkcji, takich jak połączenie lub realizacja przesyłu danych. Autor uważa, iż pewne wyszczególnienie części metod oraz zastosowanych bibliotek może pomóc niektórym osobom w prostszym znalezieniu informacji na temat poprawnie działającego połączenia z mikrokontrolerem Arduino poprzez Bluetooth.
...
Gotowy produkt składa się z dwóch pięter, z których każde pełni inną funkcję. 
Pierwsze piętro to "piętro maszynowe", gdzie umieszczone zostały moduły napędowe, takie jak silniki kątowe wraz z przekładnią oraz ogniwa zasilające w ilości dwóch wraz ze sterownikiem BMS. 
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.45\textwidth}
        ...
        \caption{Widok rzutu górnego na drugie piętro modelu. Znajdują się w nim dwa mikrokontrolery Arduino UNO oraz płytka stykowa.}
        \label{fig:zdjecie1}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
...
\begin{figure}[H]
    ...
    \caption{Fragment kodu źródłowego przedstawiający weryfikacje prawidłowego połączenia silników}
...
\newpage
\subsection{\Large Montaż ogniw wraz z układem BMS}
...
Pakiet powinien mieć wypadkowe napięcie $U=3\ldots7,5 V$, a także być gotowy na pobór prądu zwarciowego $I_{stall}=11,2 A$ w przypadku zaklinowania wszystkich kół robota. W związku z tym moc zasilania powinna wynosić $P=33,6\ldots67,2 W$.
...
\begin{enumerate}[label=(\Alph*)]
    ...
    Ze względu na dostępność, do obliczeń posłużono się ogniwem 18650 Li-ion Samsung INR18650-25R 2500mAh 20A (Rys. \ref{fig:spec-18650}). Ogniwo te ma przystępną cenę, wysoką pojemność i przede wszystkim wysoki maksymalny prąd rozładowania, daje to pewność, że w przypadku awarii i zwarcia wszystkich silników nie dojdzie do uszkodzenia baterii i potencjalnego pożaru.
    ...
    \begin{table}[H]
        ...
        \hline
        \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Układ \\ pakietu\end{tabular}} & \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Prąd obciążenia\\ na pojedyncze \\ ogniwo {[}A{]}\end{tabular}} & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}Przybliżony czas pracy\\ od 4,2 V do 3 V dla \\ charakterystyk spadków \\ napięć\end{tabular}} \\ \cline{3-4} 
                                                                                &                                                                                                            & \multicolumn{1}{c|}{{[}min{]}}                                                    & {[}h{]}                                                   \\ \hline
        1P                                                                        & 3                                                                                                          & \multicolumn{1}{c|}{47}                                                           & 0,73                                                      \\ \hline
        2P                                                                        & 1,50                                                                                                       & \multicolumn{1}{c|}{108}                                                          & 1,80                                                      \\ \hline
        3P                                                                        & 1                                                                                                          & \multicolumn{1}{c|}{144}                                                          & 2,40                                                      \\ \hline
        4P                                                                        & 0,75                                                                                                       & \multicolumn{1}{c|}{216}                                                          & 3,60                                                      \\ \hline
        \end{tabular}
        \caption{Przybliżony czas pracy pakietu ogniw względem wykresów spadków napięć.}
        \label{tab:cells-times-from-chart}
    \end{table}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|c|c|cc|}
        \hline
        \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Układ \\ pakietu\end{tabular}} & \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Prąd obciążenia\\ na pojedyncze \\ ogniwo {[}A{]}\end{tabular}} & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}Przybliżony czas pracy\\ od 4,2 V do 3 V dla \\ typowych pojemności\end{tabular}} \\ \cline{3-4} 
                                                                                &                                                                                                            & \multicolumn{1}{c|}{{[}min{]}}                                                    & {[}h{]}                                                   \\ \hline
        1P                                                                        & 3                                                                                                          & \multicolumn{1}{c|}{50}                                                           & 0,83                                                      \\ \hline
        2P                                                                        & 1,50                                                                                                       & \multicolumn{1}{c|}{100}                                                          & 1,67                                                      \\ \hline
        3P                                                                        & 1                                                                                                          & \multicolumn{1}{c|}{150}                                                          & 2,50                                                      \\ \hline
        4P                                                                        & 0,75                                                                                                       & \multicolumn{1}{c|}{200}                                                          & 3,33                                                      \\ \hline
        \end{tabular}
        \caption{Czas pracy pakietu ogniw obliczony z typowej pojemności.}
        \label{tab:cells-times-from-capacity}
    \end{table}
    ...
    Ostatecznie użyto pakiet 2S, który także jest wystarczający do zasilania. Zestaw Arduino oraz nakładkę zasilono przy pomocy powerbanka, w związku na łatwość połączenia.
    \end{enumerate}
...
%
% Status: Łukasz 50% (jeszcze coś dopiszę) || Jakub - 100%
% --------------------------------------------------------
...
Realizacja wyszukiwania dostępnych urządzeń (adresów) jest relatywnie prosta. W pętli \texttt{for} poprzez wywołanie funkcji \texttt{Wire.beginTransmission(int addr)} rozpoczynana jest próbna transmisja do potencjalnego urządzenia. Jeśli urzadzenie odpowie bez błędu, jego adres jest wyświetlany w konsoli w postaci liczby w formacie szesnastkowym.
\begin{figure}[H]
    \centering
...
We fragmencie kodu źródłowego można zauważyć wykomentowany fragment wyświetlający wartość odległości. Został on wykorzystany w celu walidacji danych odczytywanych z czujników.
...
Opuszczanie wnęki zostało zrealizowane za pomocą licznika, stąd deklaracja zmiennej \texttt{currentTime} na początku funkcji. W linii nr 55 sprawdzany jest warunek, czy czas przeznaczony na wyjazd z wnęki minął. Jeśli tak, pojazd przełącza się w tryb normalny, jeśli nie, pojazd kontynuuje jazdę prosto. 
...
Funkcja ta realizuje wysyłanie wartości odczytanej z funkcji przetwarzającej dane z sensorów. Pierwszym krokiem jest rozpoczęcie transmisji z urządzeniem o adresie \texttt{0x09} - taki adres jest typowo przydzielany dla drugiego mikrokontrolera Arduino. Następnie za pomocą funkcji \texttt{Wire.write()} odbywa się właściwy przesył danych poprzez magistralę I2C.
...
%
% Status: Jakub - 80% (? idk czy ktoś chce coś dodać)
% --------------------------------------------------------
...
Problematyczne okazało się użytkowanie więcej niż jednego czujnika laserowego przy wykorzystaniu magistrali I2C. Było to spowodowane domyślnym adresem, który był taki sam dla wszystkich czujników. 
...
% Status: 80%
% --------------------------------------------------------
Pierwszym problemem było zamontowanie samych silników, okazało się że otwory na mocowanie silników znajdowały się w innych miejscach niż było to potrzebne. W związku z tym, konieczne było wprowadzenie listwy aluminiowej, która pozwoliła na stabilne przymocowanie silników do dolnej części pojazdu.

Kolejnym wyzwaniem było mocowanie kół do silników. Początkowo użyto długich śrub, jednak okazało się, że koła są zbyt niestabilne. Aby wyeliminować luzy, wprowadzono boczne listwy aluminiowe, przez które przechodza długie gwintowane szprychy. To rozwiązanie znaczenie poprawiło stabilność kół i zminimalizowało luzy, zapewniając lepsze działanie całego układu.
...
Realizacja logiki realizującej śledzenie ściany bez elementu opuszczania wnęki była relatywnie prosta. Jednak dodanie sekwencji wyjeżdżania z wnęki okazało się wyzwaniem.

Było tak ze względu na kolejność instrukcji warunkowych. Pierwotnie jeśli robot dojeżdżał do ściany jednocześnie mając z prawej swojej strony przeszkodę, poprawnie rozpoczynał ruch w lewo. Jednak po otrzymaniu informacji z sensora o pustej przestrzeni przed, robot od razu rozpoczynał ruch w prawo, ze względu na oddalenie się od prawej strony ściany. W wyniku czego pojazd jeździł tam i z powrotem w kółko. 
...
%
% Status: Jakub - 100% (?) idk czy ktoś chce coś dodać
% --------------------------------------------------------
